...
arquivos horários, os registros de um par
$(\text{estágio}, \text{cenário})$ são acumulados em memória e
gravados em uma única operação agregada, conforme descrito na
Seção~\ref{sec:formato_resultados}; já nos arquivos diários, cada
...
(Seção~\ref{sec:metrica_banda_percebida}): em 2~nós, os blocos são
absorvidos com bloqueio mínimo pelos \textit{buffers} da pilha MPI e
...
inclui computação, E/S, comunicação e \texttt{MPI\_File\_open}/\texttt{MPI\_File\_close}. Os tempos de
I/O, comunicação e \texttt{MPI\_File\_open}/\texttt{MPI\_File\_close} são apresentados separadamente.
...
ser ocupada pelas chamadas \texttt{MPI\_File\_open}/\texttt{MPI\_File\_close}: essa parcela cresce
...
Tabela~\ref{tab:mpiio_aws}); a configuração de 4~OSTs foi obtida em
...
tempo total corresponde ao tempo de execução medido, que inclui computação,
E/S, comunicação e \texttt{MPI\_File\_open}/\texttt{MPI\_File\_close}. A eficiência é calculada em relação
à base de 2~nós.}
...
pelo paralelismo de escrita. Em 8~nós, a implementação atual ainda
