...
\subsection{Avaliação da implementação MPI-IO com escritas independentes assíncronas}
\label{sec:aws_mpiio}
Esta subseção analisa o desempenho da arquitetura proposta, baseada em
escritas independentes assíncronas com MPI-IO sobre o sistema de arquivos
...
implementação MPI-IO assíncrona no Experimento AWS. Assim como na tabela
...
\begin{table}[H]
\centering
\caption{Desempenho do Experimento AWS com a implementação MPI-IO assíncrona,
...
ganho da arquitetura proposta: a MPI-IO assíncrona passa de desempenho
...
\begin{table}[H]
\centering
\caption{Resumo comparativo entre a implementação atual e MPI-IO assíncrona no
...
implementação MPI-IO assíncrona parte de 57{,}0~Gb/s em 2~nós e cresce
...
que a da implementação MPI-IO assíncrona. Esse comportamento é coerente com o fato
...
implementação MPI-IO assíncrona sobre o Lustre.
...
assíncrona apresenta $5{,}13 \times 10^{3}$~s contra $4{,}82 \times
...
$2{,}01 \times 10^{3}$~s em 32~nós ---, a implementação MPI-IO assíncrona mantém
...
MPI-IO assíncrona permanece monotonicamente decrescente em todo o intervalo
...
Cabe observar que, na implementação MPI-IO assíncrona, o tempo de E/S torna-se uma
...
em 32~nós. Na implementação MPI-IO assíncrona, parte do tempo de simulação passa a
...
custo de coordenação, a implementação MPI-IO assíncrona já apresenta
