...
determinante para a análise de desempenho de E/S paralela. A
...
Observa-se que blocos de dados pequenos dominam a distribuição:
...
Adicionalmente, este trabalho avaliará o tempo de envio dos blocos de
dados agrupados por faixas de tamanho, de forma a analisar o impacto da
...
A distribuição observada na Figura~\ref{fig:histograma_mensagens}
está diretamente associada à organização dos arquivos de resultado
produzidos pelo SDDP. Nos experimentos, este trabalho gerou 117 arquivos
de resultado: 104 no formato horário e 13 no formato diário. Nos
arquivos horários, a aplicação acumula em memória os registros de um par
$(\text{estágio}, \text{cenário})$ e os grava em uma única operação
agregada, conforme descrito na Seção~\ref{sec:formato_resultados}; já nos
arquivos diários, a aplicação grava cada dia do mês individualmente, sem
agregação entre dias.
Essa diferença de granularidade explica o perfil observado: os
13~arquivos diários respondem pela grande maioria dos blocos
pequenos (até 1~KB), enquanto os blocos médios e grandes provêm
dos 104~arquivos horários.
