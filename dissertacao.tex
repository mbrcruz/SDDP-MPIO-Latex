...
reservatórios. O algoritmo opera em duas fases iterativas: na passada
\textit{forward}, percorre os estágios do primeiro ao último,
resolvendo cada subproblema sob a política corrente para gerar
trajetórias de operação; na passada \textit{backward}, percorre os
estágios em sentido inverso, agregando os multiplicadores duais para
construir cortes de Benders que aproximam a função de custo futuro.
Cada iteração refina a política, e o procedimento é repetido até a
convergência.

A Figura~\ref{fig:SDDP_cenarios_estagios} esquematiza a estrutura de
subproblemas resolvida a cada passada do SDDP nas configurações
adotadas nos experimentos desta dissertação: três estágios mensais e
$1\,536$ cenários, totalizando $4\,608$ subproblemas independentes por
iteração. A independência entre os cenários de um mesmo estágio é a
propriedade que viabiliza a paralelização da passada \textit{forward}:
os cenários são distribuídos entre os \textit{ranks} MPI e resolvidos
simultaneamente, de modo que o tempo total da passada fica limitado
pelo cenário mais lento de cada estágio e não pela soma sequencial.
Após a resolução, os resultados de cada subproblema --- variáveis
primais, multiplicadores duais e custos --- precisam ser persistidos
em disco, pois alimentam tanto a passada \textit{backward} subsequente
quanto análises estatísticas posteriores. É precisamente nesse ponto
de persistência massiva que se manifesta o gargalo de E/S investigado
neste trabalho: o volume agregado escrito por iteração cresce com o
número de cenários e com a granularidade dos dados produzidos, e a
estratégia de implementação dessas escritas determina diretamente a
escalabilidade da aplicação em ambientes HPC.
...
\begin{figure}[hbt]
  ...
  $(\text{estágio}, \text{cenário})$ resolvido por um \textit{rank}
  MPI; os subproblemas de um mesmo estágio são independentes entre si
  e podem ser executados em paralelo na passada \textit{forward}.}
...
\texttt{MPI\_File\_Open}/\texttt{MPI\_File\_Close}. As barras de erro indicam somente o
desvio padrão do tempo de execução total. Em 2~nós, a implementação MPI-IO
apresenta tempo total superior ao da implementação atual, pois o custo
dessas operações de coordenação e da própria camada MPI-IO ainda pesa de
forma significativa. Nas configurações com 4, 8 e 16~nós, a versão MPI-IO
...
custo da Coordenação MPI-IO passe a compor uma fração visível do tempo
...
única vez por arquivo, fora do laço de escritas. Cada
\texttt{MPI\_File\_write\_at} independente paga, no cliente, um custo
fixo de \textit{setup} da chamada e, no servidor, ao menos uma RPC ao
OSS que hospeda o OST do \textit{offset} de destino; em RPCs pequenas,
o tempo de ida-e-volta domina sobre o tempo de transferência da carga
útil. Adicionalmente, escritas concorrentes a regiões distintas do
mesmo arquivo demandam aquisição e liberação de \textit{extent locks}
pelo \textit{Lustre Distributed Lock Manager} (LDLM), gerenciados pelos
OSS, cujo custo é independente do tamanho do bloco
escrito~\cite{Pjb:02,LustreManual}. Com 384 \textit{ranks} em 16~nós
emitindo RPCs simultâneas e disputando \textit{extent locks}, esses dois
fatores combinados explicam o crescimento do tempo médio por chamada com
o número de nós. Uma confirmação experimental dirigida dessa hipótese
--- por meio de \textit{benchmark} sintético (IOR) e contadores
\texttt{lctl} do Lustre durante a execução --- é deixada como trabalho
futuro.
...
e 12,1\% em 32~nós. Embora essas operações introduzam sincronização
global, seu custo permanece menor que a contenção imposta pelo escritor
único, e a implementação MPI-IO apresenta tempo total inferior ao da
implementação atual em todas as escalas avaliadas.
