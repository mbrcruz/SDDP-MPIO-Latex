...
Experimento SDumont, em escala logarítmica. Para blocos de dados pequenos,
até 1~KB, a implementação atual permanece na ordem de microssegundos,
enquanto a implementação MPI-IO passa para a ordem de décimos de segundo.
Essa diferença indica que, nessa classe, o custo fixo de abertura,
coordenação e atualização de metadados não é amortizado pelo volume de
dados transferido. Como o SDumont utiliza um MDS/MDT único, o efeito torna
mais perceptível a pressão exercida pelo caminho de metadados do Lustre.

Nas classes médias e grandes, o comportamento se inverte. A implementação
atual passa a acumular tempos de bloqueio elevados porque os blocos de dados
pressionam o processo escritor centralizado. A implementação MPI-IO, por sua
vez, distribui as escritas pelo sistema de arquivos Lustre e reduz
substancialmente o tempo de bloqueio, sobretudo nas classes de maior volume.
Assim, a figura evidencia um ponto de troca: para blocos pequenos, o custo de
coordenação e metadados domina; para blocos maiores, a escrita paralela reduz
a contenção do escritor único.
