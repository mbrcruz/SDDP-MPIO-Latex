...
O restante deste artigo segue esta organização: a Seção~\ref{sec:fundamentacao} apresenta os conceitos fundamentais; A Seção~\ref{sec:Proposta} detalha nossa proposta. A Seção~\ref{sec:avaliacoes} traz a avaliação e a análise experimental da abordagem, enquanto na Seção~\ref{sec:relacionados} são sumarizados os principais trabalhos relacionados. Finalmente, a Seção~\ref{sec:conclusao} fornece as considerações finais e delineia direções para trabalhos futuros. \textbf{TODO: REVISAR AO FINAL}

\section{Fundamentação}~\label{sec:fundamentacao}

\subsection{MPI, comunicação e operações paralelas de arquivos}

O MPI é o padrão dominante para programação paralela baseada em troca de mensagens, fornecendo primitivas ponto a ponto (\texttt{MPI\_Send}, \texttt{MPI\_Recv}) e operações coletivas. A partir do MPI-2, passou a incluir a especificação MPI-IO, que estende o mesmo modelo de coordenação para operações paralelas de E/S~\cite{mpiCite,mpiioCite}. Comunicação e E/S não são problemas independentes: uma arquitetura que concentra resultados em um único escritor transforma a E/S em comunicação ponto a ponto, enquanto uma arquitetura baseada em MPI-IO permite que os próprios processos produtores escrevam seus resultados, deslocando o gargalo do processo escritor para o sistema de arquivos paralelo. Um aspecto relevante em aplicações \textit{bag-of-tasks} é o custo de bloqueio imposto ao remetente pelo \texttt{MPI\_Send}: quando múltiplos \textit{ranks} concorrem por um único receptor, o tempo de bloqueio cresce com o volume enviado e se agrava à medida que aumenta o número de remetentes simultâneos.

Para operações paralelas de arquivos, o MPI-IO oferece três grupos de escritas com custos distintos. Nas \emph{escritas independentes} (p.\,ex.\ \texttt{MPI\_File\_write\_at}), cada processo executa sua própria chamada informando o \textit{offset} de destino, sem exigir participação dos demais --- padrão simples e robusto para aplicações \textit{bag-of-tasks}, em que tarefas independentes terminam em momentos diferentes~\cite{Cirne:2003}. Nas \emph{escritas coletivas} (\texttt{MPI\_File\_write\_all}), todos os processos de um comunicador participam, permitindo à biblioteca reorganizar a E/S por técnicas como \textit{two-phase I/O} e \textit{data sieving}, ao custo de sincronização entre processos. Há ainda variantes \emph{não bloqueantes} (\texttt{MPI\_File\_iwrite\_at}), que iniciam a E/S e retornam antes de sua conclusão, permitindo sobrepor escrita e computação, mas exigindo que os \textit{buffers} permaneçam válidos até o término da operação.

\subsection{Sistema de arquivos paralelo Lustre}

O Lustre é um sistema de arquivos paralelo projetado para cargas massivas de HPC, cujo princípio central é a separação entre o fluxo de metadados e o de dados~\cite{Pjb:02,LustreManual}. O \textit{Metadata Server} (MDS) mantém o \textit{namespace} e localiza os dados; a partir daí, as operações de leitura e escrita ocorrem diretamente com os \textit{Object Storage Servers} (OSSs), que gerenciam os \textit{Object Storage Targets} (OSTs) --- os dispositivos onde os dados são efetivamente gravados. O paralelismo de acesso cresce com o número de OSTs. Para distribuir os dados, o Lustre emprega \textit{striping}: divide o arquivo em blocos de tamanho fixo (\textit{stripe size}) e os grava de forma circular em um subconjunto de OSTs (\textit{stripe count}), multiplicando a banda efetiva percebida pela aplicação. \textit{Stripes} maiores favorecem grandes operações sequenciais, enquanto \textit{stripes} menores distribuídos por mais OSTs beneficiam acessos concorrentes de granularidade fina.

\subsection{Ambientes computacionais}~\label{sec:ambientes}

Os experimentos foram conduzidos em dois ambientes de HPC com características distintas, que se diferenciam sobretudo na tecnologia de interconexão. Na nuvem \textbf{AWS}, as instâncias empregam o \textit{Elastic Network Adapter} (ENA), interface de rede virtual baseada em Ethernet, e o armazenamento é provido pelo Amazon FSx for Lustre~\cite{aws:ena}. Para reduzir a latência, este trabalho alocou todos os nós em uma única zona de disponibilidade, usando um \textit{Placement Group} do tipo \textit{cluster}, com o FSx provisionado na mesma zona~\cite{aws:placement}. Já o supercomputador \textbf{Santos Dumont} (SDumont), do LNCC, é um ambiente HPC tradicional com topologia fixa e interconexão InfiniBand EDR, que combina alta banda e latência muito baixa~\cite{Fds:03,RlGmmts:05}. Os experimentos utilizaram MPICH~3.2 com o dispositivo \textit{ch3} (IP sobre InfiniBand), sem explorar RDMA nativo.
...
Esta seção apresenta a avaliação comparativa entre a implementação atual de E/S e a proposta, com foco em tempo de execução, comunicação, largura de banda e tempo de bloqueio das operações de E/S~\cite{Rwe:99,Phrrbrw:09}. A base de dados corresponde ao PMO/PAR de outubro de 2023. Para a simulação horária --- foco do estudo ---, consideraram-se 1.536 cenários em ambos os experimentos (AWS e SDumont), com três estágios mensais. Para cada configuração foram executadas cinco repetições; os valores reportados correspondem à média e ao desvio padrão. Como cada \textit{rank} consome mais de 8~GB de memória residente, mapeou-se apenas um \textit{rank} por núcleo físico (sem \textit{hyper-threading}), pois, com 384~GB por nó, dobrar o número de \textit{ranks} ultrapassaria a memória física agregada. Os experimentos caracterizam um cenário de \textit{strong scaling}~\cite{Amdahl:67}: o tamanho do problema permanece fixo enquanto o número de nós aumenta.

A instrumentação, posicionada no módulo de persistência com \texttt{MPI\_Wtime}, é coerente entre as duas implementações, permitindo comparação direta. A \emph{banda agregada percebida pela aplicação} sintetiza quanto da capacidade de E/S a aplicação efetivamente aproveita: para janelas de tempo de duração fixa $\Delta$, corresponde à razão entre o volume gravado por todos os \textit{ranks} e o tempo de E/S do \textit{rank} mais lento da janela, $B_w = \left(\sum_{i=1}^{P} V_{i,w}\right) / \max_{i}\, T_{i,w}^{\text{E/S}}$, com o valor reportado sendo a média ao longo das janelas e das repetições. O denominador adota o processo mais lento, em consonância com o comportamento síncrono da aplicação ao final de cada janela.
...
As Figuras~\ref{fig:banda_aws} e~\ref{fig:tempo_aws} consolidam esse comportamento. As curvas de banda são qualitativamente opostas: a implementação atual parte de 60,5~Gb/s em 2~nós e degrada-se mais de uma ordem de grandeza, enquanto a MPI-IO cresce monotonicamente até 380,3~Gb/s. Essa diferença decorre da eliminação do processo escritor único: na arquitetura atual, todas as escritas convergem para um único nó, cuja serialização rapidamente satura; na MPI-IO, a escrita é distribuída entre os processos e os múltiplos OSTs. Quanto ao tempo total, a implementação atual entra em regime de saturação a partir de 16~nós e torna-se contraproducente em 32~nós, ao passo que a MPI-IO permanece monotonicamente decrescente. Na MPI-IO, as chamadas \texttt{MPI\_File\_open}/\texttt{MPI\_File\_close} (coluna \emph{open/close} da Tabela~\ref{tab:mpiio_aws}) crescem de 42,5~s (0,5\%) em 2~nós para 147,3~s (11,6\%) em 32~nós. Trata-se de \emph{overhead fixo}: executado uma única vez por execução, não cresce com o número de cenários nem de estágios. Sua fração percentual aparece inflada na tabela e na Figura~\ref{fig:tempo_aws} porque estes experimentos usam apenas três estágios mensais, o que mantém curto o tempo total; em cargas reais de operação programada, com horizonte plurianual, o custo absoluto permanece o mesmo enquanto o tempo total cresce em ordem de magnitude, diluindo essa parcela a níveis desprezíveis.
...
\begin{table}[ht]
\centering
\caption{Experimento SDumont: desempenho das implementações atual e MPI-IO (banda percebida, tempo de E/S, tempo de texttt{MPI\_File\_open}/\texttt{MPI\_File\_close}, tempo total e eficiência). Valores em média\,$\pm$\,desvio padrão de cinco repetições, exceto eficiência (pontual); eficiência relativa à base de 2~nós; a coluna \emph{open/close} aplica-se apenas à MPI-IO.}
...
A Figura~\ref{fig:banda_sdumont} confirma o contraste de \emph{comportamento}: a implementação atual preserva banda elevada até 8~nós, mas colapsa a partir de 16~nós (entre 2,5 e 4,2~Gb/s), sintoma da saturação do escritor central sob o Lustre compartilhado; a MPI-IO exibe perfil monotonicamente crescente. \textbf{TODO: EXPANDIR DISCUSSÂO}
