...
%Esta seção apresenta a avaliação comparativa entre a implementação atual de E/S e a proposta, com foco em tempo de execução, comunicação, largura de banda e tempo de bloqueio das operações de E/S~\cite{Rwe:99,Phrrbrw:09}. A base de dados corresponde ao PMO/PAR de outubro de 2023. Os experimentos avaliaram dois ambientes distintos: a nuvem \textbf{AWS}, com instâncias baseadas em \textit{Elastic Network Adapter} (ENA) e Amazon FSx for Lustre alocados em zona única sob \textit{Placement Group} tipo \textit{cluster}~\cite{aws:ena, aws:placement}; e o supercomputador \textbf{Santos Dumont} (SDumont/LNCC), infraestrutura HPC tradicional com interconexão InfiniBand EDR de alta banda e baixa latência~\cite{Fds:03,RlGmmts:05}, utilizando MPICH 3.2 com o dispositivo \textit{ch3} sobre IP, sem RDMA nativo. 

%Para a simulação horária --- foco do estudo ---, consideraram-se 1.536 cenários em ambos os experimentos (AWS e SDumont), com três estágios mensais. Para cada configuração foram executadas cinco repetições; os valores reportados correspondem à média e ao desvio padrão. Como cada \textit{rank} consome mais de 8~GB de memória residente, mapeou-se apenas um \textit{rank} por núcleo físico (sem \textit{hyper-threading}), pois, com 384~GB por nó, dobrar o número de \textit{ranks} ultrapassaria a memória física agregada. Os experimentos caracterizam um cenário de \textit{strong scaling}~\cite{Amdahl:67}: o tamanho do problema permanece fixo enquanto o número de nós aumenta.
