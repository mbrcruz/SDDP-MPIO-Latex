...
As Figuras~\ref{fig:banda_aws} e~\ref{fig:tempo_aws} consolidam esse comportamento. As curvas de banda são qualitativamente opostas: a implementação atual parte de 60,5~Gb/s em 2~nós e degrada-se mais de uma ordem de grandeza, enquanto a MPI-IO cresce monotonicamente até 380,3~Gb/s. Essa diferença decorre da eliminação do processo escritor único: na arquitetura atual, todas as escritas convergem para um único nó, cuja serialização rapidamente satura; na MPI-IO, a escrita é distribuída entre os processos e os múltiplos OSTs. Quanto ao tempo total, a implementação atual entra em regime de saturação a partir de 16~nós e torna-se contraproducente em 32~nós, ao passo que a MPI-IO permanece monotonicamente decrescente. Na MPI-IO, as chamadas \texttt{MPI\_File\_open}/\texttt{MPI\_File\_close} (coluna \emph{open/close} da Tabela~\ref{tab:mpiio_aws}) crescem de 42,5~s (0,5\%) em 2~nós para 147,3~s (11,6\%) em 32~nós. Esse custo é pago uma única vez por execução, na abertura e no fechamento dos arquivos (início e fim), e \emph{independe do tamanho da carga}: não cresce com o número de cenários nem de estágios, dependendo apenas do número de nós que participam da operação coletiva. Por isso sua fração percentual aparece inflada na tabela e na Figura~\ref{fig:tempo_aws}, já que estes experimentos usam apenas três estágios mensais, o que mantém curto o tempo total. Mantido o mesmo número de nós, essa parcela se dilui em cargas reais de operação programada com horizonte plurianual: ao quadruplicar o horizonte (de 3 para 12 estágios), por exemplo, todas as componentes da decomposição --- computação, comunicação e E/S --- crescem proporcionalmente, enquanto o custo de abertura e fechamento permanece inalterado, tornando-se desprezível frente ao tempo total.
