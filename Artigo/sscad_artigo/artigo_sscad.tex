...
Como direções para trabalhos futuros, destacam-se a reestruturação do modelo de persistência dos arquivos diários para mitigar o overhead de E/S granular; a exploração de interconexões de altíssima velocidade (InfiniBand HDR/NDR, AWS EFA) com bufferização alinhada ao \textit{stripe} do Lustre; a reavaliação de escritas coletivas com \textit{two-phase I/O}; e a extensão da arquitetura à fase de geração de cortes do SDDP.
