...
\title{Escalando a E/S do SDDP em Ambientes HPC: da Escrita Serializada às Escritas Paralelas com MPI-IO sobre Lustre\thanks{Esse projeto foi parcialmente financiado com recursos de bolsas de produtividade do CNPq (PQ-C: 304308/2025-0) e da FAPERJ (JCNE: E-26/204.481/2025). Recursos computacionais do SDumont alocados ao projeto wnnParIO}}

\author{Marcelo Barros da Cruz\orcidlink{0009-0008-5751-1238}\inst{1}, Claudio Luis de Amorim\orcidlink{0000-0003-1651-1967}\inst{1}, Diego Leonel Cadette Dutra\orcidlink{0000-0003-4262-7242}\inst{1}}
...
% =============================================================
\subsection{Experimento AWS}~\label{sec:exp_aws}
...
Cabe ressalvar que, apesar dos ganhos de E/S, a eficiência paralela permanece bem abaixo do ideal linear (44\% na AWS e 41\% no SDumont em 32~nós). Isso é esperado no regime de \textit{strong scaling}: com o problema de tamanho fixo, cada processo resolve menos cenários à medida que se adicionam nós, e custos quase constantes da computação --- sobretudo os de acesso à memória --- deixam de ser amortizados. Esse gargalo computacional, distinto do gargalo de E/S aqui tratado, foge ao escopo deste trabalho.
...
Mais do que os ganhos expressivos de desempenho e largura de banda observados em 32 nós, o resultado mais robusto deste estudo é o contraste qualitativo de comportamento: enquanto a estratégia centralizada colapsa devido à saturação do escritor, a arquitetura com MPI-IO exibe um perfil de escalabilidade monotonicamente crescente em ambas as infraestruturas. Isso demonstra não apenas a eficácia técnica da abordagem, mas também a sua portabilidade entre ecossistemas heterogêneos de computação de alto desempenho. Ademais, a análise do número de OSTs reforça que o dimensionamento da camada de armazenamento (\textit{stripe count} e \textit{stripe size}) deve ser tratado como parâmetro estrutural de projeto.

%Como direções para trabalhos futuros, destacam-se a reestruturação do modelo de persistência dos arquivos diários para mitigar o overhead de E/S granular; a exploração de interconexões de altíssima velocidade (InfiniBand HDR/NDR, AWS EFA) com bufferização alinhada ao \textit{stripe} do Lustre; a reavaliação de escritas coletivas com \textit{two-phase I/O}; o diagnóstico das variações observadas no tempo de computação para uma mesma configuração no ambiente AWS; e a extensão da arquitetura à fase de geração de cortes do SDDP.

Como direções para trabalhos futuros, destacam-se a reestruturação do modelo de persistência dos arquivos diários para mitigar o overhead de E/S granular; a exploração de interconexões de altíssima velocidade (InfiniBand HDR/NDR, AWS EFA) com bufferização alinhada ao stripe do Lustre; a reavaliação de escritas coletivas com two-phase I/O; a investigação da piora pontual observada em 4 nós no ambiente AWS (Seção~\ref{sec:exp_aws}), onde o tempo total da implementação MPI-IO superou o da implementação atual apesar do ganho já mensurável de banda — indício de que fatores fora do módulo de E/S, possivelmente variações no tempo de computação entre repetições, dominaram esse ponto específico da curva; e a extensão da arquitetura à fase de geração de cortes do SDDP.
